...
\begin{frame}{Análise Comparativa}
    Para realizar uma análise de performance do Cilk Plus, foram escolhidos 
    dois problemas triviais e comparados sua execução com o \textit{OpenMP}:
\newline
\begin{columns}
\begin{column}{0.5\textwidth}
\centering
...
\begin{enumerate}
    ...
    \item Tamanho da matriz: 10000 x 10000
    \item Valor do escalar: aleatório
    \item Variação nas \textit{threads}
\end{enumerate}
\end{column}
\begin{column}{0.6\textwidth}
GRÁFICO DE TEMPOS
...
%\begin{groupplot}[
%    group style={group size=1 by 1,	horizontal sep=1.7cm,}, 			ylabel={Eficiência},
%	xlabel={Taxa},
%%   xmin = 0, xmax = 30,
%    xtick=data,
...
%    footnotesize,
%%    legend columns=5,
%    legend style={at={(0.5,1.18)},anchor=north},
%    cycle list name ={my chart colors},
%%    x post scale=3
%]
%\nextgroupplot
%\addplot table[x=taxa, y=eficiencia]{dados/escalabilidadeFraca.dat};
%\addlegendentry{Escalabilidade Fraca}
%\end{groupplot}
%\end{tikzpicture}
%\end{center}
